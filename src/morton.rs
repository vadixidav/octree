...
//
// This file is a derivative of the one found here:
// https://github.com/swiftcoder/isosurface/blob/675538f46ce0423165774c2ceb2a2bf2a14d6b2b/src/morton.rs
...
const HIGHEST_LEVEL_MASK: u64 = 0xE000_0000_0000_0000;
...
impl Morton {
    ...
    pub fn child(self, which: u8) -> Option<Self> {
        if self.0 & HIGHEST_LEVEL_MASK == 0 {
            Some(Morton((self.0 << 3) | u64::from(which)))
        } else {
            None
        }
    }

    /// Get all of the 8 child nodes of this octree node.
    pub fn children(self) -> Option<impl Iterator<Item = Self>> {
        if self.0 & HIGHEST_LEVEL_MASK == 0 {
            Some((0..8).map(move |i| Morton((self.0 << 3) | i)))
        } else {
            None
        }
    }
